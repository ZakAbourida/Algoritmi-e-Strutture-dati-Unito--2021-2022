...
MyArray *my_array_create() {
	...
	my_array->array = (void**)malloc(INITIAL_CAPACITY * sizeof(Record*));
...
MyArray *my_array_add(MyArray *my_array, void *element) {
	...
	if (my_array->size >= my_array->array_capacity) {
		my_array->array_capacity = 2 * my_array->array_capacity;
		my_array->array = (void**)realloc(my_array->array, (unsigned long)(my_array->array_capacity) * sizeof(Record*));
		if (my_array->array == NULL) {
